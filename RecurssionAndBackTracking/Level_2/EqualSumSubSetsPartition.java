...
public class EqualSumSubSetsPartition {

    public static void solution(int[] arr, int vidx, int n, int k, int[] subsetSum, int nos, ArrayList<ArrayList<Integer>> ans) {
        if (vidx == arr.length) {
            if (nos == k) {
                boolean flag = true;
        ...
        for (int i = 0; i < ans.size(); i++) {
            if (ans.get(i).size() > 0) {
                ...
                solution(arr, vidx + 1, n, k, subsetSum, nos, ans);
                subsetSum[i] -= arr[vidx];
                ans.get(i).remove(ans.get(i).size() - 1);
            } else {
                ans.get(i).add(arr[vidx]);
                subsetSum[i] += arr[vidx];
                solution(arr, vidx + 1, n, k, subsetSum, nos + 1, ans);
                subsetSum[i] -= arr[vidx];
                ans.get(i).remove(ans.get(i).size() - 1);
                break;
            }
